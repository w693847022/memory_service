...
class GroupsService:
    ...
    async def update_group_config(
        ...
        if group in RESERVED_FIELDS:
            return ResponseBuilder.error(f"组名 '{group}' 与系统配置字段冲突").to_dict()
        if group not in groups:
            valid_groups = ", ".join(sorted(groups.keys()))
            return ResponseBuilder.error(f"无效的分组类型: {group} (支持: {valid_groups})").to_dict()
