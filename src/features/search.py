...
def _handle_search_errors(api_name):
    """搜索函数错误处理装饰器."""
    def decorator(func):
        def wrapper(*args, **kwargs):
            try:
                ...
                return {"success": False, "error": f"{api_name} API timeout"}
            except requests.exceptions.RequestException as e:
                return {"success": False, "error": f"{api_name} API error: {str(e)}"}
...
@_handle_search_errors("Stack Overflow")
